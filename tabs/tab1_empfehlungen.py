...
def render(stock_df, selected_model, groq_key, get_berlin_time_str):
    ...
    if not stock_df.empty:
        ...
        st.info("📂 Lade zuerst dein Depot hoch, um die Aktien bewerten zu lassen.")
        return
    ...
    with col_time:
        ...
        if depot_time:
            st.caption(f"🕒 Letzte Analyse: **{depot_time}**")
    ...
    if depot_text and len(depot_text.strip()) > 15:
        ...
        st.info("Klicke oben auf '🤖 Depot-Bewertung & Empfehlungen jetzt analysieren', um den detaillierten Bericht zu berechnen.")
