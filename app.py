...
with st.sidebar:
  ...
  if st.session_state.my_portfolio:
    for idx, item in enumerate(list(st.session_state.my_portfolio)):
      with st.container():
        ...
        with c_a:
          st.write(
              f"**{item.get('name', item['ticker'])}** ({item['ticker']})"
          )
          st.caption(
  ...
  if GROQ_KEY:
    ...
    selected_model = st.selectbox(
        "Aktives Modell", available_models, index=0
    )
  else:
...
with tab7:
  ...
  if not stock_df.empty and len(stock_df) >= 2:
    ...
    with cd1:
      duel_a = st.selectbox("Wähle Aktie A:", stock_df["Ticker"].tolist(), index=0)
    with cd2:
      duel_b = st.selectbox("Wähle Aktie B:", stock_df["Ticker"].tolist(), index=1)
